...
import { insertUserSchema, insertProfileSchema, insertResumeSchema, 
  insertFormHistorySchema, insertExtensionSettingsSchema, insertStatisticsSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put(`${apiPrefix}/resumes/:id/default`, authenticate, async (req, res) => {
    try {
      const resumeId = parseInt(req.params.id);
      const userId = req.body.userId;
      
      const success = await storage.setDefaultResume(resumeId, userId);
  ...
  app.post(`${apiPrefix}/form-history`, authenticate, async (req, res) => {
    try {
      ...
      if (stats) {
        ...
        const successfulApps = (stats.successRate * stats.applicationsFilled / 100) + 
  ...
  app.get(`${apiPrefix}/extension-settings`, authenticate, async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Server error fetching extension settings' });
    }
  });
  ...
  app.put(`${apiPrefix}/extension-settings`, authenticate, async (req, res) => {
    try {
      ...
      if (!existingSettings) {
        ...
        const newSettings = await storage.createExtensionSettings(newSettingsData);
        return res.json(newSettings);
      }
      
      // Update existing settings
      const updatedSettings = await storage.updateExtensionSettings(userId, settingsData);
      res.json(updatedSettings);
    } catch (error) {
      if (error instanceof ZodError) {
        return handleZodError(error, res);
      }
      res.status(500).json({ error: 'Server error updating extension settings' });
    }
  });
